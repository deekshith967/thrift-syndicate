...
export default function AdminDashboard() {
  // Reactive single source of truth product list
  ...
  const metrics = useMemo(() => {
    ...
    return { totalProducts, inStockProducts, totalValue, categoriesCount };
  }, [productList]);
  ...
  return (
    <div className="min-h-screen bg-[#F8F8F8] flex flex-col lg:flex-row text-[#111111] font-sans pt-16">
      ...
      {/* Mobile Top Bar */}
      <div className="lg:hidden bg-[#111111] text-white p-4 flex items-center justify-between sticky top-16 z-30">
      ...
      <main className="flex-1 p-6 sm:p-10 max-w-7xl">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
          <div>
            <h1 className="font-display font-black text-3xl sm:text-4xl uppercase tracking-tight text-[#111111]">
              {activeTab === 'overview' ? 'Dashboard Overview' : 'Manage Products'}
            </h1>
            <p className="text-xs text-neutral-500 mt-1">
              Add, edit, filter, and monitor your vintage product inventory.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
          ...
          <div className="bg-white p-5 rounded-2xl border border-neutral-200 shadow-xs space-y-2">
            ...
            <p className="font-display font-black text-3xl text-emerald-700">{metrics.inStockProducts}</p>
          </div>

          <div className="bg-white p-5 rounded-2xl border border-neutral-200 shadow-xs space-y-2">
            <div className="flex items-center justify-between text-neutral-500">
              <span className="text-xs font-bold uppercase tracking-wider">Catalog Value</span>
              <DollarSign size={18} className="text-neutral-800" />
            </div>
            <p className="font-display font-black text-3xl text-[#111111]">₹{metrics.totalValue.toLocaleString()}</p>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-neutral-200 shadow-xs space-y-2">
            ...
            <p className="font-display font-black text-3xl text-[#111111]">{metrics.categoriesCount}</p>
          </div>
        </div>

        {/* Tab View: Overview vs Products Table */}
        {activeTab === 'overview' ? (
          <div className="bg-white rounded-3xl border border-neutral-200 p-8 space-y-6 shadow-xs">
            <h3 className="font-display font-bold text-xl uppercase">Quick Stock Summary</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <h4 className="text-xs font-bold uppercase text-neutral-500">Categories Breakdown</h4>
                <div className="space-y-2">
                  {categories.filter(c => c !== 'All').map((cat) => {
                    const count = productList.filter(p => p.category === cat).length;
                    return (
                      <div key={cat} className="flex items-center justify-between p-3 bg-neutral-50 rounded-xl border border-neutral-100 text-xs">
                        <span className="font-bold text-neutral-800">{cat}</span>
                        <span className="font-mono bg-white px-2.5 py-0.5 rounded-md border border-neutral-200 font-bold">{count} items</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="space-y-3">
                <h4 className="text-xs font-bold uppercase text-neutral-500">Admin Actions</h4>
                <div className="space-y-3">
                  <button
                  ...
                  >
                    <span>Go to Product Management Table</span>
